...
delBtn.addEventListener('click', () => {
    if (btnForm.lastChild) {
        btnForm.removeChild(btnForm.lastChild);
    } else {
        info.innerHTML = "No child elements to remove!";
    }
})

var rowId = 1;

function updateTable(type, cost) {
    ...
    row.innerHTML = `
        ...
        <td id="del-row"><button onclick="deleteRow(${rowId})">del</button></td>
        `;
...
function updateRowIds() {
    ...
    for (let i = 0; i < rows.length; i++) {
        const rowId = i + 1;
        rows[i].setAttribute('id', `row${rowId}`);
